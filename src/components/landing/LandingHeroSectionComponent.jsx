          ...
          <div className="absolute -bottom-4 left-4 right-4 rounded-2xl border border-gray-100 bg-white/95 p-4 shadow-lg backdrop-blur-sm sm:left-auto sm:right-6 sm:w-72 lg:-left-6 lg:bottom-8">
            ...
            miniProducts?.map((p) => (
              <Link
                ...
                className="relative size-14 overflow-hidden rounded-lg bg-gray-100 ring-1 ring-gray-100 transition hover:scale-105"
              >
                {p.imageUrl ? (
                  <Image src={p.imageUrl} alt="" fill sizes="56px" className="object-cover" />
                ) : (
                  <span className="flex size-full items-center justify-center text-xs text-gray-400">◇</span>
                )}
              </Link>
            ))
          ) : status === "loading" ? (
            ...
            <p className="text-xs text-gray-400 italic">Login to view products</p>
          )}
